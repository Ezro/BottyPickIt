...
import math
import botty_pickit
# from botty_pickit import BOTTY_ITEMS_BY_BASE_THEN_QUALITY, FoundItem
...
class ItemFinder:
    ...
    def search_for_base_item(self, inp_img: np.ndarray, base_item: BaseItem) -> list[Item]:
        img = inp_img[:,:,:]
        start = time.time()
        item_text_clusters = self._item_cropper.crop(img, 7)
        item_list = []
        for cluster in item_text_clusters:
            ...
            best_score = None
            item = None
            for quality in botty_pickit.BOTTY_ITEMS_BY_BASE_THEN_QUALITY[base_item.value]:
                ground_ref_data = botty_pickit.BOTTY_ITEMS_BY_BASE_THEN_QUALITY[base_item.value][quality].ground_ref_data
                if cropped_input.shape[1] > ground_ref_data.shape[1] and cropped_input.shape[0] > ground_ref_data.shape[0]:
                    # sanity check if there is any color overlap of template and cropped_input
                    grayscale = cv2.cvtColor(cropped_input, cv2.COLOR_BGR2GRAY)
                    _, mask = cv2.threshold(grayscale, 0, 255, cv2.THRESH_BINARY)
                    hist = cv2.calcHist([cropped_input], [0, 1, 2], mask, [8, 8, 8], [0, 256, 0, 256, 0, 256])
                    hist_result = cv2.compareHist(hist, hist, cv2.HISTCMP_CORREL)
                    same_type = hist_result > 0.0 and hist_result is not np.inf
                    if same_type:
                        result = cv2.matchTemplate(cropped_input, ground_ref_data, cv2.TM_CCOEFF_NORMED)
                        _, max_val, _, max_loc = cv2.minMaxLoc(result)
                        if max_val > self._min_score:
                            if (best_score is None or max_val > best_score):
                                best_score = max_val
                                ...
                                # TODO: After cropping the "cropped_input" with "cropped_item", check if "cropped_input" might need to be
                                #       checked for other items. This would solve the issue of many items in one line being in one cluster
                                roi = [max_loc[0], max_loc[1], ground_ref_data.shape[1], ground_ref_data.data.shape[0]]
                                cropped_item = cut_roi(cropped_input, roi)
                                grayscale = cv2.cvtColor(cropped_item, cv2.COLOR_BGR2GRAY)
                                _, mask = cv2.threshold(grayscale, 0, 255, cv2.THRESH_BINARY)
                                hist = cv2.calcHist([cropped_item], [0, 1, 2], mask, [8, 8, 8], [0, 256, 0, 256, 0, 256])
                                ...
                                same_type = hist_result > 0.65 and hist_result is not np.inf
                                if same_type:
                                    center = (int(max_loc[0] + x + int(ground_ref_data.shape[1] * 0.5)), int(max_loc[1] + y + int(ground_ref_data.shape[0] * 0.5)))
                                    found_base_item = base_item.value
                                    found_quality = quality
                                    roi = [max_loc[0] + x, max_loc[1] + y, ground_ref_data.shape[1], ground_ref_data.shape[0]]
                                    found_item = botty_pickit.FoundItem(
                                        center=center,
                                        base_item=found_base_item,
    ...
    def search_for_base_item_and_quality(self, inp_img: np.ndarray, base_item: BaseItem, quality: ItemQuality) -> list[Item]:
        img = inp_img[:,:,:]
        start = time.time()
        item_text_clusters = self._item_cropper.crop(img, 7)
        item_list = []
        for cluster in item_text_clusters:
            ...
            best_score = None
            item = None
            ground_ref_data = botty_pickit.BOTTY_ITEMS_BY_BASE_THEN_QUALITY[base_item.value][quality.value].ground_ref_data
            if cropped_input.shape[1] > ground_ref_data.shape[1] and cropped_input.shape[0] > ground_ref_data.shape[0]:
                # sanity check if there is any color overlap of template and cropped_input
                grayscale = cv2.cvtColor(cropped_input, cv2.COLOR_BGR2GRAY)
                _, mask = cv2.threshold(grayscale, 0, 255, cv2.THRESH_BINARY)
